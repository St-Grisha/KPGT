...
def extract_features(args):
    ...
    mol_dataset = MoleculeDataset(
        root_path=args.data_path, dataset=args.dataset, dataset_type=None
    )
    loader = DataLoader(
        mol_dataset,
        batch_size=32,
    ...
    model.load_state_dict(
        {k.replace("module.", ""): v for k, v in torch.load(args.model_path).items()}
    )
    fps_list = []
    ...
    np.savez_compressed(
        f"{args.data_path}/{args.dataset}/kpgt_{args.config}.npz",
        fps=np.array(fps_list),
    )
    print(
        f"The extracted features were saving at {args.data_path}/{args.dataset}/kpgt_{args.config}.npz"
    )
