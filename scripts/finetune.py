...
def finetune(args):
    ...
    model.predictor = get_predictor(
        d_input_feats=config["d_g_feats"] * 3,
    ...
    trainer = Trainer(
        ...
        device=device,
        model_name="LiGhT",
        label_mean=train_dataset.mean.to(device)
        if train_dataset.mean is not None
        else None,
        label_std=train_dataset.std.to(device)
        if train_dataset.std is not None
        else None,
    )
    best_train, best_val, best_test = trainer.fit(
