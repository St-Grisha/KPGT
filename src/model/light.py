...
class TripletTransformer(nn.Module):
    ...
    def forward(self, g, triplet_h, dist_attn, path_attn):
        ...
        g.edata["a"] = (
            g.edata["node_attn"]
            + dist_attn.reshape(len(g.edata["node_attn"]), -1, 1)
        ...
        g.edata["he"] = (
            (g.edata["he"].view(-1, self.n_heads, self.d_feats // self.n_heads))
            * g.edata["sa"]
        ).view(-1, self.d_feats)
...
class AtomEmbedding(nn.Module):
    ...
    def forward(self, pair_node_feats, indicators):
        ...
        pair_node_h[
            indicators == VIRTUAL_ATOM_FEATURE_PLACEHOLDER, 1, :
        ] = self.virtual_atom_emb.weight  # .half()
...
class BondEmbedding(nn.Module):
    ...
    def forward(self, edge_feats, indicators):
        ...
        edge_h[
            indicators == VIRTUAL_BOND_FEATURE_PLACEHOLDER
        ] = self.virutal_bond_emb.weight  # .half()
...
class LiGhTPredictor(nn.Module):
    ...
    def forward(self, g, fp, md):
        ...
        # Predict
        return (
            self.node_predictor(triplet_h[g.ndata["mask"] >= 1]),
