...
class Evaluator:
        ...
        if not y_true.shape[1] == self.n_tasks:
            raise RuntimeError(
                "Number of tasks for {} should be {} but {} given".format(
                    self.name, self.n_tasks, y_true.shape[1]
                )
            )
    ...
    def eval(self, y_true, y_pred, valid_ids=None):
        y_true, y_pred, valid_ids = self._parse_and_check_input(
                ...
                rmse_list.append(
                    np.sqrt(
                        (
                            (
                                y_true[is_labeled, i]
                                - (y_pred[is_labeled, i] * self.std[i] + self.mean[i])
                            )
                            ** 2
                        ).mean()
                    )
    ...
    def _eval_mae(self, y_true, y_pred):
        ...
        for i in range(y_true.shape[1]):
            ...
            if (self.mean is not None) and (self.std is not None):
                mae_list.append(
                    mean_absolute_error(
                        y_true[:, i], y_pred[:, i] * self.std[i] + self.mean[i]
                    )
                )
            else:
